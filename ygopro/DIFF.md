...
+ When duel is created, a `CreateGame` message will be sent to duel. That message doesn't exist in the original ygopro, it is only for plugin can catch that time.
+ The ygocore duel is created when the room is created (SingleDuel::new), unlike the original which creates it in `TpResult`.
