...
async fn run_tcp_server(
    ...
    let duel = ygopro::cli::build_duel_host(
        server_arguments.host_info,
        server_arguments.replay_mode,
        server_arguments.seeds,
    );
    run_local_server(listener, duel, shutdown_receiver).await
}

async fn run_local_server(
    listener: TcpListener,
    mut duel: impl RoomProvider<
        ctos::Message,
        ygopro_data::complex::Complex<ygopro_data::message::stoc::Message>,
    > + Send
    + 'static,
...
fn parse_server_arguments(arguments: &[String]) -> Result<ServerArguments, i32> {
    ...
    let host_info = HostInfo {
        lflist: deck_manager
            .get_lflist_by_index(arguments[1].parse().unwrap_or(0))
            .map(|l| l.hash)
            .unwrap_or(0),
    ...
    let replay_mode: ReplayMode =
        ReplayMode::from_bits_retain(arguments[11].parse::<u32>().unwrap_or(0));
